...
class NusClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._rssi_updated_at: float | None = None
    ...
    @property
    def rssi_age_s(self) -> float | None:
        if self._rssi_updated_at is None:
            return None
        return round(max(0.0, time.monotonic() - self._rssi_updated_at), 1)
    ...
    async def connect(self) -> None:
        ...
        def match_target(dev: Any, advertisement: Any) -> bool:
            matched = _address_matches(getattr(dev, "address", ""), self.address)
            if matched:
                value = getattr(advertisement, "rssi", None)
                if value is not None:
                    self._last_rssi = int(value)
                    self._rssi_updated_at = time.monotonic()
            return matched
...
class RingSoundClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._command_counts: DefaultDict[int, int] = defaultdict(int)
    ...
    def protocol_diagnostics(self) -> dict[str, Any]:
        now = time.monotonic()
        return {
            "rx_chunk_count": self._rx_chunk_count,
            "rx_byte_count": self._rx_byte_count,
            "packet_count": self._packet_count,
            "protocol_error_count": self._protocol_error_count,
            "last_rx_age_s": (
                round(now - self._last_rx_at, 1)
                if self._last_rx_at is not None
                else None
            ),
            "last_packet_age_s": (
                round(now - self._last_packet_at, 1)
                if self._last_packet_at is not None
                else None
            ),
            "command_counts": {
                f"0x{command:04X}": count
                for command, count in sorted(self._command_counts.items())
            },
        }

    async def connect(self) -> None:
        self._clear_receive_state()
        self._reset_protocol_diagnostics()
    ...
    def _on_rx(self, data: bytes) -> None:
        ...
        for packet in packets:
            ...
            self._command_counts[packet.command] += 1
    ...
    def _reset_protocol_diagnostics(self) -> None:
        self._rx_chunk_count = 0
        self._rx_byte_count = 0
        self._packet_count = 0
        self._protocol_error_count = 0
